...
PowerShell -ExecutionPolicy ByPass -File .\stop.ps1 -noprofile
set ret=%ERRORLEVEL%
echo Stop.bat return value: %ret%
)
...
rem ***************************************
:FAILOVER
IF %CLP_FACTOR% == "GROUPFAILOVER" (
echo "Error caused the group to failover."
)
